...
class GeMapFullHead(DETRHead):
    ...
    def __init__(self,
        ...
        super(GeMapFullHead, self).__init__(
    ...
    def forward(self, mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        ...
        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            ...
            outputs = self.transformer(
                ...
                lidar_feat,
                bev_queries,
                object_query_embeds[:self.num_vec*self.num_pts_per_vec],
                self.bev_h,
                self.bev_w,
                grid_length=(self.real_h / self.bev_h,
                             self.real_w / self.bev_w),
        ...
        for lvl in range(hs.shape[0]):
            ...
            outputs_coord, outputs_pts_coord = self.transform_box(tmp, num_vec=self.num_vec)
    ...
    def chamfer_distance(self, src, tgt):
        distance = torch.cdist(src, tgt)
        src2tgt_distance, _ = torch.min(distance, dim=2)
        tgt2src_distance, _ = torch.min(distance, dim=1)
        return 0.5 * (torch.mean(src2tgt_distance, dim=-1) + torch.mean(tgt2src_distance, dim=-1))
    ...
    def compute_inter_geometrics(self, x, offset_x):
        ...
        crosses = crosses.flatten()
        # for each pair of instances, compute angles between offset vectors
        return length, dots, crosses
